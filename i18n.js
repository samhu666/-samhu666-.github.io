...
const translations = {
  zh: {
    ...
    'workDetail.back': '← 返回作品案例',
    'workDetail.card1.metaTitle': '引擎機構模型設計｜FS MEC STUDIO',
    'workDetail.card1.metaDesc': '以 SolidWorks 完整建構渦輪噴射引擎內部機構，剖面呈現風扇、壓縮機與渦輪葉片的多級裝配關係，驗證複雜旋轉機構的建模與干涉檢查能力。',
    'workDetail.card1.desc2': '專案涵蓋多級旋轉零件的裝配關係定義、干涉檢查與剖面渲染輸出，最終交付完整的 3D 模型檔案與工程圖，供後續模擬分析與製造參考使用。',
    'workDetail.card2.metaTitle': '無人機機臂結構優化｜FS MEC STUDIO',
    'workDetail.card2.metaDesc': '使用 ANSYS 進行拓樸優化與應力模擬，機臂減重 23% 同時維持原有強度。',
    'workDetail.card2.desc2': '透過反覆迭代拓樸優化模型並比對應力與變形分析結果，找出材料分佈的最佳配置，在滿足飛行載重與安全係數的前提下大幅降低整機重量。',
    'workDetail.card3.metaTitle': '桌上型循線機器人套件｜FS MEC STUDIO',
    'workDetail.card3.metaDesc': '為教育訓練設計的教學用循線機器人，含開源韌體與教材文件，已授權給 3 個單位使用。',
    'workDetail.card3.desc2': '套件包含機構設計、感測器配置與韌體程式，並搭配循序漸進的教學文件，方便講師在課程中直接使用，目前已授權給 3 個教育訓練單位導入課程。',
    'workDetail.card4.metaTitle': '義肢手部關節原型｜FS MEC STUDIO',
    'workDetail.card4.metaDesc': '與生醫工程團隊合作，以樹脂列印製作三代關節原型，協助驗證握力與活動範圍。',
    'workDetail.card4.desc2': '歷經三代迭代，逐步調整關節結構與連桿設計，以樹脂 3D 列印快速驗證握力表現與活動範圍，為後續正式量產設計提供關鍵測試數據。',
  ...
  en: {
    ...
    'workDetail.back': '← Back to Work',
    'workDetail.card1.metaTitle': 'Turbojet Engine Model｜FS MEC STUDIO',
    'workDetail.card1.metaDesc': 'A complete internal mechanism of a turbojet engine modeled in SolidWorks, with sectional views showing the multi-stage assembly of fan, compressor, and turbine blades — demonstrating advanced modeling and interference-checking capability for complex rotating assemblies.',
    'workDetail.card1.desc2': 'The project covers defining assembly relationships between multi-stage rotating parts, interference checking, and sectional rendering output, delivering complete 3D model files and engineering drawings for downstream simulation and manufacturing reference.',
    'workDetail.card2.metaTitle': 'Drone Arm Structural Optimization｜FS MEC STUDIO',
    'workDetail.card2.metaDesc': 'Topology optimization and stress simulation in ANSYS reduced arm weight by 23% while maintaining original strength.',
    'workDetail.card2.desc2': 'Through repeated iterations of the topology optimization model alongside stress and deformation analysis, we identified the optimal material distribution — significantly reducing overall weight while meeting flight load and safety factor requirements.',
    'workDetail.card3.metaTitle': 'Desktop Line-Following Robot Kit｜FS MEC STUDIO',
    'workDetail.card3.metaDesc': 'A line-following robot kit designed for training purposes, including open-source firmware and course materials, licensed to 3 organizations.',
    'workDetail.card3.desc2': 'The kit includes mechanism design, sensor configuration, and firmware code, paired with step-by-step course materials that instructors can use directly in class — currently licensed to 3 training organizations.',
    'workDetail.card4.metaTitle': 'Prosthetic Hand Joint Prototype｜FS MEC STUDIO',
    'workDetail.card4.metaDesc': 'Developed with a biomedical engineering team, resin-printed through three generations of joint prototypes to validate grip strength and range of motion.',
    'workDetail.card4.desc2': 'Across three generations of iteration, we progressively refined the joint structure and linkage design, using resin 3D printing to rapidly validate grip strength and range of motion — providing key test data for the subsequent production-ready design.',
